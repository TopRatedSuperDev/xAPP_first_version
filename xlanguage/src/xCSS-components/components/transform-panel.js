        ...
        <div className="control-panel-horizontal-layout">
          <div className="control-panel-horizontal-layout-item">
            ...
            {Object.keys(transform.translate).map(coordinate => {
              return <FormGroup label={`Translate${coordinate.toLocaleUpperCase()}`} inline key={coordinate} labelFor={coordinate}>
                <NumericInput
                  disabled={disabled}
            ...
            {Object.keys(transform.scale).map(coordinate => {
              return <FormGroup label={`Scale${coordinate.toLocaleUpperCase()}`} inline key={coordinate} labelFor={coordinate}>
            ...
            <div className='transform-skew-container'>
              {Object.keys(transform.skew).map(coordinate => {
                return <FormGroup key={coordinate} label={`Skew${coordinate.toLocaleUpperCase()}`} inlkey={coordinate} labelFor={coordinate}>
                  <div>
                    <AnglePicker id={coordinate} angle={transform.skew[coordinate]} setAngle={value => onValueChange(value, 'skew', coordinate)} />
                  </div>
                </FormGroup>
              })}
            </div>
