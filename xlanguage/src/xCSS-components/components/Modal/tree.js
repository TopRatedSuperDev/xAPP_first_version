...
function mapDataToTreeData(data) {
    ...
    for (let key in data) 
    {
      ...
      if (key ==="attributes") {
        ...
        // })
        value.forEach(item => {
          if (item.name === "class" || item.name === "src" || item.name === "alt" || item.name === "id" || item.name === "style") {
            node.attributes.push(item);
          }
        });
...
const toNode = t => {
  switch (t?.nodeType) {
    ...
    default: return  HtmlNode(t.tagName, t.attributes, t.children)
  }
}
...
class Tree extends React.Component {
  ...
  constructor(props) {
    ...
    if(document.getElementsByClassName('canvas-panel')[0].children.length === 0) {
      MyDiv1 = document.getElementsByClassName('canvas-panel')[0];
    } else {
      MyDiv1 = document.getElementsByClassName('canvas-panel')[0].firstChild;
    }
    ...
    return (
      <div className="wrapper_tree">
        <div className="bar_wrapper_tree">
          ...
          {/* <button onClick={() => {useCoreDataStore.getState().addNewTreeElement(this.state.treeData);this.props.callback();}} >Create Tree</button> */}
          <button onClick={() => {useCoreDataStore.getState().updateNewTreeElement(this.state.treeData);this.props.callback();}} >Update Tree</button>
                      ...
                      onClick={() =>
                        this.setState((state) => ({
                          treeData: addNodeUnderParent({
                            ...
                            newNode: {
                              nodeType: 1,
                              attributes: [{name:"id", value:null}, {name:"class", value:"canvas-item"}] ,
                              title: `DIV`,
                              tagName: 'DIV',
                              children:[],
                            },
                    ...
                    onClick={() =>
                      this.setState((state) => ({
                        treeData: addNodeUnderParent({
                          ...
                          newNode: {
                            nodeType: 1,
                            attributes: [{name:"id", value:null}, {name:"class", value:"canvas-item"}, {name:"alt", value:"upload the source file"}] ,
                            title: `IMG`,
                            tagName: 'IMG',
                            children:[],
                          },
